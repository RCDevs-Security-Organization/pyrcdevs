...
def test_wrong_host() -> None:
    with pytest.raises(aiohttp.client_exceptions.ClientConnectorDNSError) as excinfo:
        asyncio.run(OpenOTPSoap("wrong_host", 8443, verify_mode=ssl.CERT_NONE).status())
...
def test_wrong_port() -> None:
    ...
    exception_msg = get_full_exception_msg(excinfo)
    assert 'aiohttp.client_exceptions.ClientConnectorError: Cannot connect to host' in exception_msg
...
def test_p12_api_key_together() -> None:
    ...
    exception_msg = get_full_exception_msg(excinfo)
    assert (
        "Client certificate and API key cannot be used together!" in exception_msg
    )
