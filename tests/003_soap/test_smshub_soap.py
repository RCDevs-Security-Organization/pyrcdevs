...
async def test_status() -> None:
    ...
    """
    response = await smshub_soap_api.status()
...
async def test_send() -> None:
    ...
    response = await smshub_soap_api.send("username", "password", [SMS_MOBILE], "test")
    ...
    response = await smshub_soap_api.send(
    ...
    response = await smshub_soap_api.send(
        RANDOM_STRING, "password", [SMS_MOBILE], "test"
    )
    ...
    response = await smshub_soap_api.send(
        "username", RANDOM_STRING, [SMS_MOBILE], "test"
    )
    ...
    response = await smshub_soap_api.send("username", "password", [], "test")
    assert all(prefix in response for prefix in ("code", "error", "message"))
    assert response["code"] == "0"
    ...
    response = await smshub_soap_api.send("username", "password", [SMS_MOBILE], "")
    ...
    with pytest.raises(TypeError) as excinfo:
        # noinspection PyTypeChecker
        await smshub_soap_api.send("username", "password", [SMS_MOBILE], "test", "Flash")
    assert str(excinfo).startswith("<ExceptionInfo TypeError('type_ is not SMSType')")


@pytest.mark.skip("Avoid using SMS credit")
@pytest.mark.asyncio
async def test_sign() -> None:
    """
    ...
    # Testing a wrong static username
    response = await smshub_soap_api.sign(
        RANDOM_STRING,
        "password",
        [SMS_MOBILE],
        "test",
    ...
    # Testing an empty list of mobile phones
    response = await smshub_soap_api.sign("username", "password", [], "test")
    assert all(prefix in response for prefix in ("code", "error", "message"))
    assert response["code"] == "0"
    assert response["error"] == "BadRequest"
    assert response["message"] == MSG_INVALID_SMS_REQUEST
    ...
    # Testing an empty list text
    response = await smshub_soap_api.sign("username", "password", [SMS_MOBILE], "")
